...
from ims_python_helper import ImsHelper, DEFAULT_IMS_API_URL
...
def create_parent_parser(program_name):
    ...
    parent_parser.add_argument(
        '--s3-host', type=str,
        default=os.environ.get('S3_HOST', None),
        help='Host of S3 instance, e.g. https://s3.shasta.local:8080. Uses S3_HOST environment variable as a default.'  # noqa: E501
    )
    parent_parser.add_argument(
    ...
    while True:
        ...
        if not token:
            ...
            if attempt_timeout != sleep_ceiling:
                attempt_timeout += 1
            LOGGER.info(
                "Unable to obtain token from auth service, retrying in %s "
                "seconds" % (attempt_timeout)
            )
        else:
            break
    return session
