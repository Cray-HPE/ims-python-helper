...
class ImsHelper(object):
    ...
    """
    def __init__(
        self, ims_url=DEFAULT_IMS_API_URL, session=None, s3_access_key=None,
        s3_secret_key=None, s3_host=None, s3_bucket=None, **kwargs
    ):
    ...
    def _ims_job_patch_resultant_image_id(self, ims_job_id, result_id):
        ...
        LOGGER.info(
            "PATCH %s resultant_image_id=%s", url, ims_job_id, result_id
        )
    ...
    def image_upload_artifacts(
        ...
        ret = {
            'result': 'success',
            'ims_image_record': None,
            'ims_image_artifacts': []
        }

        # Create a skeleton image to get an image id
        ret['ims_image_record'] = self._ims_image_create(image_name)
        ...
        if boot_parameters:
            to_upload.append(('application/vnd.cray.image.parameters.boot', key % 'boot_parameters', boot_parameters[0]))  # noqa: E501
